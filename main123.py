...
def select_questions(questions_list, num_questions_per_field=None):
    """
    Selects questions from the list. If num_questions_per_field is None,
    it selects all questions from each field. Otherwise, it selects
    the specified number of questions per field.
    """
    selected = []
    fields = {}
    ...
    for field in fields:
        ...
        if num_questions_per_field is None:
            selected.extend(fields[field]) # Include all questions for this field
        else:
...
if 'level1_questions' not in st.session_state:
    # Set num_questions_per_field to None to include all questions
    st.session_state.level1_questions = select_questions(all_questions_l1, num_questions_per_field=None)
...
if 'level2_questions' not in st.session_state:
    # Set num_questions_per_field to None to include all questions
    st.session_state.level2_questions = select_questions(all_questions_l2, num_questions_per_field=None)
...
if st.session_state.quiz_state == "start":
    ...
    st.write(f"Level 1 will have {len(st.session_state.level1_questions)} questions.")
    st.write(f"Level 2 will have {len(st.session_state.level2_questions)} questions.")
